   ...
   public  class SchedulerCenterServer : ISchedulerCenter
    {
        ...
        /// </summary>
        /// <returns></returns>
        public async Task<string> StartScheduleAsync()
        {
        ...
        /// </summary>
        /// <returns></returns>
        public async Task<string> StopScheduleAsync()
        {
           // var result = new ResponseOutput<string>();
            try
            {
        ...
        /// <returns></returns>
        public async Task<string> AddScheduleJobAsync(TasksQz tasksQz)
        {
            //var result = new ResponseOutput<string>();
            ...
            if (tasksQz != null)
            {
                try
                {
                    ...
                    {
                        //   result.NotOk($"该任务计划已经在执行:【{tasksQz.Name}】,请勿重复启动！");
                        //  return result;
                        return $"该任务计划已经在执行:【{tasksQz.Name}】,请勿重复启动！";
                    }
                    #region 设置开始时间和结束时间
                    ...
                    //result.Ok($"启动任务:【{tasksQz.Name}】成功");
                    //return result;
                    return $"启动任务:【{tasksQz.Name}】成功";
                }
                catch (Exception ex)
                {
                    // result.NotOk($"任务计划异常:【{ex.Message}】");
                    //return result;
                    return $"任务计划异常:【{ex.Message}】";
                }
            }
            else
            {
                //result.NotOk($"任务计划不存在:【{tasksQz?.Name}】");
                //return result;
                return $"任务计划不存在:【{tasksQz?.Name}】";
            }
        }
        ...
        /// </summary>
        /// <returns></returns>
        public async Task<string> StopScheduleJobAsync(TasksQz sysSchedule)
        {
            //var result = new ResponseOutput<string>();
            try
            {
                ...
                if (!await _scheduler.Result.CheckExists(jobKey))
                {
                    //result.NotOk($"未找到要暂停的任务:【{sysSchedule.Name}】");
                    //return result;
                    return $"未找到要暂停的任务:【{sysSchedule.Name}】";
                }
                else
                {
                    await this._scheduler.Result.PauseJob(jobKey);
                    //result.Ok($"暂停任务:【{sysSchedule.Name}】成功");
                    //return result;
                    return $"暂停任务:【{sysSchedule.Name}】成功";
                }
            }
            catch (Exception)
            {
        ...
        /// <param name="tasksQz"></param>
        /// <returns></returns>
        public async Task<string> ResumeJob(TasksQz tasksQz)
        {
            //var result = new ResponseOutput<string>();
            try
            {
                ...
                if (!await _scheduler.Result.CheckExists(jobKey))
                {
                    //  result.NotOk($"未找到要重新的任务:【{tasksQz.Name}】,请先选择添加计划！");
                    // return result;
                    return $"未找到要重新的任务:【{tasksQz.Name}】,请先选择添加计划！";
                }
               ...
               ((CronTriggerImpl)trigger).MisfireInstruction = MisfireInstruction.CronTrigger.DoNothing;
                ...
                //result.Ok($"恢复计划任务:【{tasksQz.Name}】成功");
                //return result;
                return $"恢复计划任务:【{tasksQz.Name}】成功";
            }
            catch (Exception)
            {
                throw;
