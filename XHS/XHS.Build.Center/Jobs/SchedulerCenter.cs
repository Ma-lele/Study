...
namespace XHS.Build.Center.Jobs
{
    public class SchedulerCenter : ISchedulerCenter
    {
        ...
        /// </summary>
        /// <returns></returns>
        public async Task<IResponseOutput> StartScheduleAsync()
        {
            var result = new ResponseOutput<string>();
            try
            {
                ...
                if (!this._scheduler.Result.IsStarted)
                {
                    ...
                    result.Ok(string.Empty, "任务调度开启成功");
                    return result;
                }
                else
                {
                    result.NotOk(string.Empty, "任务调度已经开启");
                    return result;
                }
            }
            catch (Exception)
        ...
        /// </summary>
        /// <returns></returns>
        public async Task<IResponseOutput> StopScheduleAsync()
        {
            var result = new ResponseOutput<string>();
            try
            {
                if (!this._scheduler.Result.IsShutdown)
                {
                    ...
                    result.Ok(string.Empty, "任务调度停止成功");
                    return result;
                }
                else
                {
                    result.NotOk(string.Empty, "任务调度已经停止");
                    return result;
                }
            }
            catch (Exception)
        ...
        /// <returns></returns>
        public async Task<IResponseOutput> AddScheduleJobAsync(TasksQz tasksQz)
        {
            var result = new ResponseOutput<string>();
            ...
            if (tasksQz != null)
            {
                try
                {
                    ...
                    {
                        result.NotOk(string.Empty, $"该任务计划已经在执行:【{tasksQz.Name}】,请勿重复启动！");
                        return result;
                    }
                    #region 设置开始时间和结束时间
                    ...
                    result.Ok(string.Empty, $"启动任务:【{tasksQz.Name}】成功");
                    return result;
                }
                catch (Exception ex)
                {
                    result.NotOk(string.Empty, $"任务计划异常:【{ex.Message}】");
                    return result;
                }
            }
            else
            {
                result.NotOk(string.Empty, $"任务计划不存在:【{tasksQz?.Name}】");
                return result;
            }
        }
        ...
        /// </summary>
        /// <returns></returns>
        public async Task<IResponseOutput> StopScheduleJobAsync(TasksQz sysSchedule)
        {
            var result = new ResponseOutput<string>();
            try
            {
                ...
                if (!await _scheduler.Result.CheckExists(jobKey))
                {
                    result.NotOk(string.Empty, $"未找到要暂停的任务:【{sysSchedule.Name}】");
                    return result;
                }
                else
                {
                    await this._scheduler.Result.PauseJob(jobKey);
                    result.Ok(string.Empty, $"暂停任务:【{sysSchedule.Name}】成功");
                    return result;
                }
            }
            catch (Exception)
        ...
        /// <param name="tasksQz"></param>
        /// <returns></returns>
        public async Task<IResponseOutput> ResumeJob(TasksQz tasksQz)
        {
            var result = new ResponseOutput<string>();
            try
            {
                ...
                if (!await _scheduler.Result.CheckExists(jobKey))
                {
                    result.NotOk(string.Empty, $"未找到要重新的任务:【{tasksQz.Name}】,请先选择添加计划！");
                    return result;
                }
                ...
                result.Ok(string.Empty, $"恢复计划任务:【{tasksQz.Name}】成功");
                return result;
            }
            catch (Exception)
            {
        ...
        /// <returns></returns>
        public async Task<TriggerState> GetJobState(TasksQz model)
        {
            JobKey jobKey = new JobKey(model.Id.ToString(), model.JobGroup);
