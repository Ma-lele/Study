...
{
    public class WXDYJiangYinJob : JobBase, IJob
    {
        private readonly ILogger<WXDYJiangYinJob> _logger;
        ...
        public WXDYJiangYinJob(ILogger<WXDYJiangYinJob> logger, IBaseRepository<BaseEntity> baseRepository, ISpecialEqpService specialEqpService)
        {
            _logger = logger;
        ...
        {
            string SUCCESS = "0000";
            string APPID = "sysDzmnzyoiuh9lz663";
            string SECRET = "PLM66dunbZ0cVgfD";
            string AES = "qyb7SzSv189mzb82";
            string url = "jyzj.jsdayun.cn:8019";
            Dictionary<string, object> asciiDic = new Dictionary<string, object>();
            string[] arrKeys;
            JObject tokenjob = new JObject();
            string token = HttpNetRequest.HttpGet("http://wx.admin.xhs-sz.com:9035/api/specialeqp/getwxdytoken?appid=" + APPID+ "&SECRET=" + SECRET+ "&AES=" + AES + "&url=" + url);
            ...
            try
            {
                ...
                //for (int i = 0; i < 0; i++)
                {
                    ...
                    string rtdUrl = string.Format("http://{0}/rest/DustNoise/addRealTimeData/{1}/{2}", url, APPID, sign);
                    ...
                    {
                        token = HttpNetRequest.HttpGet("http://wx.admin.xhs-sz.com:9035/api/specialeqp/getwxdytoken?isForce=true"+ "&appid=" + APPID + "&SECRET=" + SECRET + "&AES=" + AES + "&url=" + url);
            ...
            if (towerDt != null && towerDt.Rows.Count > 0)
            {
                try
                {
                    ...
                    for (var i = 0; i < towerDt.Rows.Count; i++)
                    {
                        ...
                        string towerUrl = string.Format("http://{0}/rest/Tower/Info/{1}/{2}", url, APPID, sign);
            ...
            if (towerDt != null && towerDt.Rows.Count > 0)
            {
                try
                {
                    ...
                    for (var i = 0; i < towerDt.Rows.Count; i++)
                    {
                        ...
                        string towerUrl = string.Format("http://{0}/rest/Tower/addRealTimeData/{1}/{2}", url, APPID, sign);
                        string response = UHttp.Post(towerUrl, JsonConvert.SerializeObject(asciiDic), UHttp.CONTENT_TYPE_JSON);
                        JObject jo = JObject.Parse(response);
                        if (Convert.ToString(jo["flag"]) == "0006" || Convert.ToString(jo["flag"]) == "0007" || Convert.ToString(jo["flag"]) == "0010")
                        {
            ...
            if (ElevatorDt != null && ElevatorDt.Rows.Count > 0)
            {
                try
                {
                    ...
                    for (var i = 0; i < ElevatorDt.Rows.Count; i++)
                    {
                        ...
                        string towerUrl = string.Format("http://{0}/rest/Elevator/Info/{1}/{2}", url, APPID, sign);
                        string response = UHttp.Post(towerUrl, JsonConvert.SerializeObject(asciiDic), UHttp.CONTENT_TYPE_JSON);
                        JObject jo = JObject.Parse(response);
                        if (Convert.ToString(jo["flag"]) == "0006" || Convert.ToString(jo["flag"]) == "0007" || Convert.ToString(jo["flag"]) == "0010")
                        {
            ...
            if (ElevatorDt != null && ElevatorDt.Rows.Count > 0)
            {
                try
                {
                    ...
                    for (var i = 0; i < ElevatorDt.Rows.Count; i++)
                    {
                        ...
                        string towerUrl = string.Format("http://{0}/rest/Elevator/addRealTimeData/{1}/{2}", url, APPID, sign);
                        string response = UHttp.Post(towerUrl, JsonConvert.SerializeObject(asciiDic), UHttp.CONTENT_TYPE_JSON);
                        JObject jo = JObject.Parse(response);
                        if (Convert.ToString(jo["flag"]) == "0006" || Convert.ToString(jo["flag"]) == "0007" || Convert.ToString(jo["flag"]) == "0010")
                        {
