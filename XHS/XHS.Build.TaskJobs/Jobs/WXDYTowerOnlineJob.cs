...
{
    public class WXDYTowerOnlineJob : JobBase, IJob
    {
        private readonly IConfiguration _configuration;
        private readonly ILogger<WXDYTowerOnlineJob> _logger;
        ...
        public WXDYTowerOnlineJob(IConfiguration configuration, ILogger<WXDYTowerOnlineJob> logger, ISpecialEqpService specialEqpService, IEmployeeService employeeService, IFileService fileService, IEmployeeCareerService employeeCareerService)
        {
        ...
        public async Task Run(IJobExecutionContext context, int jobid)
        {
            ...
            var towerDt = await _specialEqpService.GetWXDYSpecialList("1","");
            if (towerDt != null && towerDt.Rows.Count > 0)
            {
                ...
                {
                    string run_status = "1";
                    var Rtd = await _specialEqpService.GetLastOneMinFirst(Convert.ToString(towerDt.Rows[i]["secode"]));
                    if (Rtd == null)//最近的一分钟内未找到实时数据，则是已下线
                    {
                        run_status = "1";
                        Rtd = await _specialEqpService.GetLastOneFirst(Convert.ToString(towerDt.Rows[i]["secode"]));
                        if (Rtd == null)
                        {
                            _logger.LogInformation("塔吊" + Convert.ToString(towerDt.Rows[i]["secode"]) + "设备未获取到实时数据");
                            continue;
                        }
                    }
                    else
                    ...
                    //    continue;
                    //}
                    //var sedata = JsonConvert.DeserializeObject<TowerCraneRealDataInput>(HttpUtility.HtmlDecode(Rtd.sedata));
                    //if (string.IsNullOrEmpty(sedata.DriverCardNo))
                    //{
                    //    _logger.LogInformation("塔吊" + Rtd.secode + "设备获取到人员身份证为空");
                    //    continue;
                    //}
                    if (towerDt.Rows[i].IsNull("ID") || string.IsNullOrEmpty(towerDt.Rows[i]["ID"].ToString()))
                    {
                        continue;
                    }
                    //员工信息
                    var employee = await _employeeService.QueryById(towerDt.Rows[i]["ID"]);
                    if (employee == null)
                    {
                        _logger.LogInformation("塔吊" + Rtd.secode + "设备" + towerDt.Rows[i]["ID"] + "未获取到员工信息");
                        continue;
                    }
                    var Files = await _fileService.GetFileListByLindId(employee.ID);
                    if (!Files.Any())
                    {
                        _logger.LogInformation("塔吊" + Rtd.secode + "设备" + towerDt.Rows[i]["ID"] + "未获取到员工照片");
                        continue;
                    }
                    var Careers = await _employeeCareerService.Query(c => c.Papertype == "1");
                    if (!Careers.Any())
                    {
                        _logger.LogInformation("塔吊" + Rtd.secode + "设备" + towerDt.Rows[i]["ID"] + "未获取到员工证书");
                        continue;
                    }
                    ...
                    towerParam["device_type"] = Convert.ToString(towerDt.Rows[i]["device_type"]);
                    ...
                    towerParam["id_card"] = towerDt.Rows[i]["ID"].ToString();
                    ...
                    string towerUrl = string.Format("http://{0}/rest/Tower/addWorkData/{1}/{2}", _configuration.GetSection("WXDY").GetValue<string>("PushUrl"), APPID, sign);
                    string response = UHttp.Post(towerUrl, JsonConvert.SerializeObject(towerParam), UHttp.CONTENT_TYPE_JSON);
                    if (string.IsNullOrEmpty(response))
                    {
                        _logger.LogError("塔吊设备人员信息：" + Rtd.secode + "URL:" + towerUrl + "请求远程错误");
                        continue;
                    }
                    JObject jo = JObject.Parse(response);
                    if (Convert.ToString(jo["flag"]) != SUCCESS)
                    {
                        _logger.LogError("塔吊设备人员信息：" + Rtd.secode + "URL:" + towerUrl + "请求返回：" + response);
                    }
                    else
                    {
                        _logger.LogInformation("塔吊" + Rtd.secode + "设备人员数据推送成功");
                    }
                }
            }
        }
