...
{
    public class SmartUpLoadMinutejianyeJob : JobBase, IJob
    {
        private readonly ILogger<SmartUpLoadMinutejianyeJob> _logger;
        private readonly IAqtUploadService _aqtUploadService;
        private readonly SiteCityNanjingToken _cityToken;
        private readonly IOperateLogService _operateLogService;
        public SmartUpLoadMinutejianyeJob(ILogger<SmartUpLoadMinutejianyeJob> logger, IOperateLogService operateLogService, SiteCityNanjingToken cityToken, IHpSystemSetting systemSettingService, IAqtUploadService aqtUploadService)
        {
            _logger = logger;
        ...
        public async Task Run(IJobExecutionContext context, int jobid)
        {
            ...
            string result = "";
            //对接数据获取
            DataSet ds = await _aqtUploadService.GetListsForNanjingMinute();        //分钟
                            ...
                            try
                            {
                                ...
                                else if (api.Contains("Hoistinterface/HoistHistory"))
                                {
                                    ...
                                    if (jso.ContainsKey("sedata"))
                                    {
                                        ...
                                        jso.Add("endTime", DateTime.Now.AddMinutes(2).ToString("yyyy-MM-dd HH:mm:ss"));
