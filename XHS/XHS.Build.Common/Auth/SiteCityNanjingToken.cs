...
{
    public class SiteCityNanjingToken
    {
        JObject Values = new JObject();
        Dictionary<string, object> ParamsValues = new Dictionary<string, object>();
        private readonly ICache _cache;
        private readonly ILogger<SiteCityNanjingToken> _logger;
        private readonly IConfiguration _configuration;
        private static readonly Mutex mutex = new Mutex();
        private static readonly Mutex mutexcenter = new Mutex();
        public SiteCityNanjingToken(ICache cache, ILogger<SiteCityNanjingToken> logger, IConfiguration configuration)
        {
            _cache = cache;
            _logger = logger;
        ...
        public string getSiteCityToken(string url, string account, string password)
        {
            ...
            string key = string.Format(CacheKey.CenterNetToken, "NanJingJianyeSiteCityToken:" + account);
            ...
            if (string.IsNullOrEmpty(token))
            {
                try
                {
                    ...
                    string retString = "";
                    retString = HttpNetRequest.PostForm("http://180.101.232.135/epoint-web/rest/oauth2/token", ParamsValues, null);
                    ...
                    if (!string.IsNullOrEmpty(retString))
                    {
                        ...
                        if (resObj != null && (int)resObj["status"]["code"] == 1)
                        {
                            ...
                            _cache.Set(key, accessToken, TimeSpan.FromMinutes(Convert.ToDouble(resObj["custom"]["expires_in"]) - 60));
                            mutexcenter.ReleaseMutex();
                            return accessToken;
