...
{
    public class SiteCityXuzhouToken
    {
        private readonly ICache _cache;
        private readonly ILogger<SiteCityXuzhouToken> _logger;
        private readonly IConfiguration _configuration;
        private static readonly Mutex mutex = new Mutex();
        private static readonly Mutex mutexcenter = new Mutex();
        public SiteCityXuzhouToken(ICache cache, ILogger<SiteCityXuzhouToken> logger, IConfiguration configuration)
        {
            _cache = cache;
            _logger = logger;
        ...
        {
            string key = string.Format(CacheKey.CenterNetToken, "CityXuzhouToken:"+ account);
            ...
            if (string.IsNullOrEmpty(token))
            {
                try
                {
                    ...
                    retString = UHttp.Post(Url, jsonObject.ToString(), UHttp.CONTENT_TYPE_JSON);               
                    ...
                    if (!string.IsNullOrEmpty(retString))
                    {
                        ...
                        if (resObj != null && resObj.Code == 200)
                        {
                            ...
                            _cache.Set(key, accessToken, TimeSpan.FromMinutes(Convert.ToDouble(720)));
                            mutexcenter.ReleaseMutex();
                            return accessToken;
                        }
                        else
